...
public class ZombieAI : MonoBehaviour
{
    ...
    private void Start()
    {
        // Sets zombieDeath to the ZombieDeath script on the zombie
        zombieDeath = GameObject.Find("Zombie001").GetComponent<ZombieDeath>();
    }

    // Update is called once per frame
    void Update()
    {
        // Set distance variable to be distance from enemy to player
    ...
    // Makes zombie move towards player and attack then when in range
    void Attack()
    {
        // If zombie isn't attacking or within range, and isn't dead:
        if (attackTrigger == false && distance >= 1.5f && zombieDeath.isDead == false)
        {
